...
class ThuVienPhapLuatCrawlerSpider(BaseSpider):

    privacy_keyworks = PRIVACY_KEYWORKS

    name = "thuvienphapluat_crawler"
    allowed_domains = ["thuvienphapluat.vn"]
    start_urls = [
        (
            "https://thuvienphapluat.vn/hoi-dap-phap-luat/tim-tu-van?searchType=1"
            f"&q={keywork}&searchField=0"
        ) for keywork in privacy_keyworks
    ] + [
        (
            "https://thuvienphapluat.vn/phap-luat/tim-tu-van?searchType=1"
            f"&q={keywork}&searchField=0"
        )
        for keywork in privacy_keyworks
    ]
    ...
    def parse_page(self, response: Response):
        ...
        tag = "h2"
        questions = response.xpath(f"//section[@class='news-content']/{tag}")
        print(len(questions))
        if len(questions) == 0:
            tag = "strong"
            questions = response.xpath(f"//section[@class='news-content']/{tag}")
        for index, question in enumerate(questions):
            ...
            answer_elements = question.xpath(
                ...
                f"[count(preceding-sibling::{tag}) = {index+1}][not(self::p//em)]"
            )
